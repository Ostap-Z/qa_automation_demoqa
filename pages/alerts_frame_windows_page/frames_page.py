...
class FramesPage(BasePage):
    ...
    def check_frame(self, frame_num):
        ...
        if frame_num == 1:
            ...
            log.info(f"Returned data for the first frame: [{text}, {width}, {height}]")
            ...
            width = frame.get_attribute("width")
            height = frame.get_attribute("height")
            log.info(f"Second frame width, height: {width}, {height}")
            self.go_to_frame(frame)
            ...
            self.go_to_default_content()
            log.info("Switched back to the default content")
            log.info(f"Returned data for the second frame: [{text}, {width}, {height}]")
            return [text, width, height]
